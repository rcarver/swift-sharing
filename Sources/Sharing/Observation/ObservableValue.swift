...
/// However, applying the ``ObservableValue`` protocol by itself to a type doesn’t add observation
/// functionality to the type. Instead, always use the ``ObservableValue()`` macro when adding
/// observation support to a type.
#if !os(visionOS)
  public protocol ObservableValue: Perceptible {
    var _$id: ObservableValueID { get }
...
public struct ObservableValueID: Equatable, Hashable, Sendable {
  @usableFromInline
  var location: UUID {
  ...
  public static func _$id<T>(for value: T) -> Self {
    (value as? any ObservableValue)?._$id ?? Self()
  }

  @inlinable
  public static func _$id(for value: some ObservableValue) -> Self {
    value._$id
  }
...
where C.Element: ObservableValue {
...
public func _$isIdentityEqual<T>(_ lhs: T, _ rhs: T) -> Bool {
  ...
  func openCollection<C: Collection>(_ lhs: C, _ rhs: Any) -> Bool {
    guard C.Element.self is any ObservableValue.Type else {
      return false
    }
  ...
  if let lhs = lhs as? any ObservableValue, let rhs = rhs as? any ObservableValue {
...
@inlinable
public func _$willModify<T: ObservableValue>(_ value: inout T) {
  value._$willModify()
}
