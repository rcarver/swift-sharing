...
public struct ObservableValueMacro {
  ...
  static let registrarTypeName = "ObservationValueRegistrar"
  static var qualifiedRegistrarTypeName: String {
    "\(moduleName).\(registrarTypeName)"
  }

  static let idName = "ObservableValueID"
  static var qualifiedIDName: String {
    "\(moduleName).\(idName)"
  }

  static let trackedMacroName = "ObservationValueTracked"
  static let ignoredMacroName = "ObservationValueIgnored"
  ...
  private static func _expansion(
    ...
    if declaration.isClass {
      ...
      throw DiagnosticsError(
        syntax: node,
        message: "'@ObservableValue' cannot be applied to class type '\(observableType.text)'",
    ...
    if declaration.isActor {
      ...
      throw DiagnosticsError(
        syntax: node,
        message: "'@ObservableValue' cannot be applied to actor type '\(observableType.text)'",
        id: .invalidApplication
      )
    }
    ...
    declaration.addIfNeeded(
      ObservableValueMacro.registrarVariable(observableType),
      to: &declarations
    )
    declaration.addIfNeeded(ObservableValueMacro.idVariable(), to: &declarations)
    declaration.addIfNeeded(ObservableValueMacro.willModifyFunction(), to: &declarations)

    return declarations
  }
}

extension [ObservableValueCase] {
    ...
    case let .element(element, tag):
      if let parameters = element.parameterClause?.parameters, parameters.count == 1 {
        ...
        return """
          ...
          return ObservableValueID()._$tag(\(tag))
          """
      }
    ...
    case let .element(element, _):
        ...
        let parameter = parameters.first
      {
        return """
          ...
          \(ObservableValueMacro.moduleName)._$willModify(&state)
...
extension ObservableValueMacro {
  ...
  >(
    ...
    let cases = [ObservableValueCase](members: declaration.memberBlock.members)
    var getCases: [String] = []
...
extension ObservableValueMacro: MemberAttributeMacro {
  ...
  >(
    ...
    property.diagnose(
      ...
      renamed: ObservableValueMacro.ignoredMacroName,
      context: context
    )
    property.diagnose(
      attribute: "ObservationTracked",
      renamed: ObservableValueMacro.trackedMacroName,
      context: context
    )
    property.diagnose(
      attribute: "PresentationState",
      renamed: ObservableValueMacro.presentsMacroName,
      context: context
    )

    if property.hasMacroApplication(ObservableValueMacro.presentsMacroName)
      || knownSupportedPropertyWrappers.contains(where: property.hasMacroApplication)
    {
      return [
        AttributeSyntax(
          attributeName: IdentifierTypeSyntax(
            name: .identifier(ObservableValueMacro.ignoredMacroName)
          )
        )
      ]
    }
    ...
    return [
      AttributeSyntax(
        attributeName: IdentifierTypeSyntax(
          name: .identifier(ObservableValueMacro.trackedMacroName)
        )
      )
    ]
  }
  ...
  func diagnose<C: MacroExpansionContext>(
      ...
      let type = attribute.attributeName.as(IdentifierTypeSyntax.self)
    {
      context.diagnose(
        Diagnostic(
          node: attribute,
          message: MacroExpansionErrorMessage("'@\(name)' cannot be used in '@ObservableValue'"),
          fixIt: .replace(
...
public struct ObservationValueTrackedMacro: AccessorMacro {
  public static func expansion<
    Context: MacroExpansionContext,
  ...
  >(
    ...
    if property.hasMacroApplication(ObservableValueMacro.ignoredMacroName)
      || property.hasMacroApplication(ObservableValueMacro.presentationStatePropertyWrapperName)
      || property.hasMacroApplication(ObservableValueMacro.presentsMacroName)
      || knownSupportedPropertyWrappers.contains(where: property.hasMacroApplication)
    {
      return []
    }
    ...
    let getAccessor: AccessorDeclSyntax =
      """
      get {
      \(raw: ObservableValueMacro.registrarVariableName).access(self, keyPath: \\.\(identifier))
      return _\(identifier)
      }
      """
...
extension ObservationValueTrackedMacro: PeerMacro {
  public static func expansion<
    Context: MacroExpansionContext,
    Declaration: DeclSyntaxProtocol
  >(
    ...
    let storage = DeclSyntax(
      property.privatePrefixed("_", addingAttribute: ObservableValueMacro.ignoredAttribute)
    )
    return [storage]
  }
}

public struct ObservationValueIgnoredMacro: AccessorMacro {
  public static func expansion<
    Context: MacroExpansionContext,
    Declaration: DeclSyntaxProtocol
  >(
